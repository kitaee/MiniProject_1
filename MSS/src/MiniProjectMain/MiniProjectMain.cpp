...
int main()
{
    wchar_t nfwDir[MAX_PATH] = {};
    if (GetEnvironmentVariableW(L"NFW_DIR", nfwDir, MAX_PATH) > 0)
    {
        std::filesystem::path nfwBinPath = std::filesystem::path(nfwDir) / L"bin";
        DWORD pathLength = GetEnvironmentVariableW(L"PATH", nullptr, 0);
        std::wstring currentPath(pathLength, L'\0');
        GetEnvironmentVariableW(L"PATH", currentPath.data(), pathLength);
        currentPath.resize(wcslen(currentPath.c_str()));

        std::wstring updatedPath = nfwBinPath.wstring() + L";" + currentPath;
    ...
    ************************************************************************/
    ...
    for (unsigned int cnt = 1; cnt <= numOfComponents; cnt++)
    {
        ...
        if (power == _T("on")) {
            BaseManager* userManager = nIntegrator->getUserManager(dllName);
            ...
            {
                tcerr << _T("[MiniProject] Failed to start component: ") << dllName << std::endl;
            }
        }
