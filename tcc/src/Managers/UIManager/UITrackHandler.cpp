...
void UITrackHandler::initialize()
{
	timer = &(NTimer::getInstance());
	time = 0;
}

void UITrackHandler::release()
{
}

void UITrackHandler::addTime(void* arg)
{
	time++;
}
...
{
	std::shared_ptr<NOM> uiNom = userMgr->registerMsg(nomMsg->getName() + _T("_GUI"));
	registeredUIMsg.emplace(nomMsg->getInstanceID(), uiNom);
	NUInteger initTime(-1);
	uiNom->setValue(_T("LogicalTime"), &initTime);
	return uiNom;
}
...
UITrackHandler::reflectUITrack(std::shared_ptr<NOM> nomMsg)
{
	...
	NUInteger logicalTimeValue;
	if (LogicalTime == static_cast<uint32_t>(-1))
		logicalTimeValue = NUInteger(time);
	else
		logicalTimeValue = NUInteger(LogicalTime + 1);
	UINom->setValue(_T("LogicalTime"), &logicalTimeValue);
...
UITrackHandler::removeUITrack(std::shared_ptr<NOM> nomMsg)
{
	...
	NUInteger logicalTimeValue(time);
	UINom->setValue(_T("LogicalTime"), &logicalTimeValue);
	auto pathList = nomMsg->getPathList();
	for (auto itr = pathList.begin(); itr != pathList.end(); itr++)
		UINom->setValue(*itr, nomMsg->getValue(*itr));
